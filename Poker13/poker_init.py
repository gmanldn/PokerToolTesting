...
def open_db() -> sqlite3.Connection:
    ...
    The schema is created automatically the very first time.
    """
...
# ──────────────────────────────────────────────────────
def record_decision(analysis, position, tier,
                    stack_bb: int, pot: float, to_call: float,
                    board: str) -> int:
    """
    Persist a decision and return its row-id.
    Separated here so the GUI stays completely DB-agnostic.
    """
    with open_db() as db:
        cur = db.execute(
            """INSERT INTO decisions
               (position, hand_tier, stack_bb, pot, to_call, board, decision)
               VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
