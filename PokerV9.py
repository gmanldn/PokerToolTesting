...
class PokerAssistant(tk.Tk):
    ...
    def _build_gui(self):
        ...
        analysis_tab = tk.Frame(notebook, bg=C_BG)
        notebook.add(analysis_tab, text="Analysis")

        header_frame = tk.Frame(analysis_tab, bg=C_ACCENT)
        ...
        # Showdown buttons ------------------------------------------------
        btn_frame = tk.Frame(body_frame, bg=C_BG)
        btn_frame.pack(fill="x", pady=5)

        tk.Button(btn_frame, text="Mark as WON", bg="#27ae60", fg="white",
                  command=lambda: self._mark_showdown(1)).pack(side="left", padx=4)
        tk.Button(btn_frame, text="Mark as LOST", bg="#c0392b", fg="white",
                  command=lambda: self._mark_showdown(0)).pack(side="left", padx=4)

        # Quick stats tab --------------------------------------------------
        stats_tab = tk.Frame(notebook, bg=C_BG)
        notebook.add(stats_tab, text="Quick Stats")

        self.stats_text = tk.Text(stats_tab, font=("Consolas", 11),
                                  bg=C_BG, fg=C_TEXT, wrap="word",
    ...
    def refresh(self):
        ...
        if len(hole) == 2:
            ...
            self.out_body.insert("end", "Drag two cards to start analysis.", "advice")

        # quick stats
        self.stats_text.insert("end", "QUICK STATS\n", "header")
        with open_db() as db:
        ...
        if total:
            self.stats_text.insert("end",
                                   f"Assistant accuracy: {wins/total:5.1%}  ({wins}/{total})\n",
                                   "metric")
        else:
            self.stats_text.insert("end", "No results marked yet.\n", "metric")
